...
export function expectString(record, key) {
...
export function expectBoolean(record, key) {
    const value = record[key];
...
export function expectNumber(record, key) {
    const value = record[key];
...
export function expectNullableNumber(record, key) {
    const value = record[key];
...
export function decodeCanonicalBase64Url(value, byteLength, label) {
    if (value.includes('=') ||
    ...
    if (decoded.length !== byteLength || decoded.toString('base64url') !== value) {
        decoded.fill(0);
        throw new PhoneHomeError('invalid_config', `${label} must encode exactly ${String(byteLength)} bytes.`);
    }
    return decoded;
}
...
function validateApiBaseUrl(value) {
    ...
    if (url.username || url.password || url.search || url.hash) {
        throw new PhoneHomeError('invalid_config', 'apiBaseUrl cannot contain credentials, a query, or a fragment.');
    }
    const localHttpHosts = new Set(['localhost', '127.0.0.1', '[::1]']);
    if (url.protocol !== 'https:' &&
        !(url.protocol === 'http:' && localHttpHosts.has(url.hostname))) {
        throw new PhoneHomeError('invalid_config', 'apiBaseUrl must use HTTPS (HTTP is allowed only for loopback development).');
    }
    return url.toString().replace(/\/$/u, '');
}
export function parseSetupBundle(value) {
...
export function parseSetupBundleJson(json) {
    let value;
    try {
...
export function parseLocationStatus(value) {
    if (typeof value !== 'string' || !LOCATION_STATUSES.has(value)) {
...
export function parseDecryptedLocation(value) {
