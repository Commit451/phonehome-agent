...
export function defaultConfigPath(environment: NodeJS.ProcessEnv = process.env): string {
  if (environment.PHONE_HOME_CONFIG) return expandPath(environment.PHONE_HOME_CONFIG);
...
async function loadFile(path: string): Promise<AgentSetupBundle> {
  let metadata;
  try {
  ...
  if (process.platform !== 'win32' && (metadata.mode & 0o077) !== 0) {
    throw new PhoneHomeError(
      'insecure_config',
      `PhoneHome config permissions are too broad. Run: chmod 600 ${path}`,
    );
  }
  return parseSetupBundleJson(await readFile(path, 'utf8'));
}

function loadEnvironment(environment: NodeJS.ProcessEnv): AgentSetupBundle | null {
...
export async function readSetupSource(source?: string): Promise<AgentSetupBundle> {
  let serialized: string;
  if (source === undefined || source === '-') {
    if (source === undefined && process.stdin.isTTY) {
      throw new PhoneHomeError(
        'setup_required',
        'Pipe the setup bundle to this command or provide a JSON file path.',
        { exitCode: 1 },
      );
    }
