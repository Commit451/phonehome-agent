...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'result': {
      ...
      printJson(
        result.status === 'completed' ? client.decryptResult(result) : result,
        global.compact,
      );
      return;
    }
...
function parseGlobalOptions(arguments_: string[]): GlobalOptions {
  ...
  for (let index = 0; index < arguments_.length; index += 1) {
    ...
    if (argument === '--config') {
      ...
      if (!value || value.startsWith('-')) throw new UsageError('--config requires a file path.');
      if (configPath !== undefined) throw new UsageError('--config can be specified only once.');
      configPath = value;
      index += 1;
...
function parseRequestOptions(arguments_: string[]): { requestId?: string } {
  let requestId: string | undefined;
  for (let index = 0; index < arguments_.length; index += 1) {
    const argument = arguments_[index];
    if (argument !== '--request-id') throw new UsageError(`Unknown request option: ${argument}`);
    const value = arguments_[index + 1];
    if (!value || value.startsWith('-')) throw new UsageError('--request-id requires a UUID.');
    if (requestId !== undefined) throw new UsageError('--request-id can be specified only once.');
    requestId = value;
    index += 1;
  }
...
function parseLocationOptions(arguments_: string[]): {
  timeoutMs?: number;
  pollIntervalMs?: number;
  requestId?: string;
} {
  const request = parseRequestOptions(
    arguments_.filter((argument, index) => {
...
function requireNoArguments(command: string, arguments_: string[]): void {
  if (arguments_.length > 0) throw new UsageError(`${command} does not accept arguments.`);
}
...
function printError(error: unknown, compact: boolean): void {
  ...
  process.stderr.write(
    `${JSON.stringify(
      { error: { code: 'internal_error', message: 'Unexpected PhoneHome CLI failure.' } },
      null,
      compact ? undefined : 2,
    )}\n`,
  );
...
main().catch((error: unknown) => printError(error, process.argv.includes('--compact')));
